...
function ChatList({
    ...
    return [...(conversations || [])]
      ...
      .sort((a, b) => {
        const timeA = new Date(
          a.updatedAt || a.lastMessageId?.createdAt || a.createdAt || 0,
        );
        const timeB = new Date(
          b.updatedAt || b.lastMessageId?.createdAt || b.createdAt || 0,
        );
        return timeB - timeA;
      });
              ...
              <Dropdown onClick={(e) => e.stopPropagation()}>
                ...
                >
                  {isGroup && (
                    <>
                      <Dropdown.Item
                        className="rounded-2 d-flex align-items-center gap-2 py-2"
                        onClick={() => handleShowMembers(conv._id)}
                      >
                        <FiUsers size={16} className="text-primary" /> Xem thành
                        viên
                      </Dropdown.Item>
                      <Dropdown.Item
                        className="rounded-2 d-flex align-items-center gap-2 py-2"
                      ...
                      >
                        <FiEdit3 size={16} className="text-success" /> Cài đặt
                        nhóm
                      </Dropdown.Item>
                      <Dropdown.Divider />
                    </>
  ...
  return (
    <div
      className="p-3 chat-list-wrapper"
      style={{ background: "#f1f5f9", height: "100%", overflowY: "auto" }}
    >
      <style>{`
      ...
      >
        <Modal.Body className="text-center py-4 px-3">
          ...
          <div className="d-flex gap-2">
            <Button
              variant="light"
              className="flex-grow-1 rounded-3 fw-600"
